...
test_server_command_queue_consumes_in_sequence_order :: proc(t: ^testing.T) {
	...
	game.net_server_enqueue_user_cmd(&session, test_server_user_cmd(3))
	game.net_server_enqueue_user_cmd(&session, test_server_user_cmd(1))
	game.net_server_enqueue_user_cmd(&session, test_server_user_cmd(2))
...
test_server_command_queue_ignores_duplicates_and_stale :: proc(t: ^testing.T) {
	session := game.NetServerSession{last_processed_cmd_sequence = 5}
	game.net_server_enqueue_user_cmd(&session, test_server_user_cmd(4))
	game.net_server_enqueue_user_cmd(&session, test_server_user_cmd(5))
	game.net_server_enqueue_user_cmd(&session, test_server_user_cmd(6))
	game.net_server_enqueue_user_cmd(&session, test_server_user_cmd(6))
...
test_server_command_queue_overflow_keeps_newest :: proc(t: ^testing.T) {
	session: game.NetServerSession
	for sequence: u32 = 1; sequence <= game.NET_SERVER_PENDING_USER_CMDS + 5; sequence += 1 {
		game.net_server_enqueue_user_cmd(&session, test_server_user_cmd(sequence))
	}

	testing.expect_value(t, session.pending_cmd_count, game.NET_SERVER_PENDING_USER_CMDS)
	testing.expect_value(t, session.pending_cmds[0].sequence, u32(6))
	testing.expect_value(t, session.pending_cmds[game.NET_SERVER_PENDING_USER_CMDS - 1].sequence, u32(game.NET_SERVER_PENDING_USER_CMDS + 5))
}

@(test)
test_server_command_queue_batch_deduplicates :: proc(t: ^testing.T) {
	state := new(game.NetServer)
	defer free(state)
	...
	game.net_server_enqueue_user_cmds(state, 0, cmds)
...
test_server_command_queue_empty_consume_uses_no_new_command :: proc(t: ^testing.T) {
	session := game.NetServerSession{last_input_cmd = test_server_user_cmd(9), has_last_input_cmd = true}
	_, ok := game.net_server_consume_user_cmds(&session)
